...
- 建立 morning briefing（每日 06:00 SGT）：今日行程、昨日工作摘要、Polaris 目標提醒、重要未讀信件
- 建立 evening review（每日 18:00 SGT）：今日完成事項、明日行程預覽、與 Polaris 目標對齊的反思提問
- 透過 macOS launchd 定時排程，呼叫 `claude -p`（headless mode）生成內容
- 資料收集透過既有 MCP tools，Telegram 發送透過 Bot API（shell wrapper 直接 curl）
...
- **Dependencies**: `claude` CLI、既有 MCP tools（資料收集）、Telegram Bot API（發送）
...
| Telegram Bot API | Shell wrapper (curl) | 發送 briefing 訊息（不依賴 MCP plugin） |
